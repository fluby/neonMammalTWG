...
recapsNdiv <- ddply(recapturedInds(divData, groupingVariables), c(groupingVariables), summarize, recapsN = n_distinct(tagID, na.rm = TRUE))
...
percDataPath2nt10 <- dataPath2 %>% group_by(year, siteID, boutID) %>% summarise(n10 = length(which(recapsN >= 10)), nPlots = length(recapsN)) %>% mutate(perc102nt = (n10/nPlots)*100)
